class MerchantsController < ApplicationController
  ...
  def index
    @merchant = Merchant.includes(:openings, :specializations).all
    apply_filter(params)
    @merchant =  @merchant.order((sort_column == 'name' ? 'merchants.name' : sort_column) + ' ' + sort_direction)
  end
  ...
  def apply_filter(params)
    ...
    if params[:session_rng].present?
      if params[:session_rng].eql?('60')
        ...
        @merchant = @merchant.joins(:openings).where('(openings.end_time-openings.start_time) < INTERVAL ?', "#{params[:price_rng]} minutes")
      end
    end
